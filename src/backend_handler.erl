...
%% For now just make these hard coded
%% -define(BAND_CENTER_10M,    28850000).
%% -define(BAND_CENTER_12M,    24940000).
%% -define(BAND_CENTER_15M,    21225000).
%% -define(BAND_CENTER_17M,    18118000).
%% -define(BAND_CENTER_20M,    14175000).
%% -define(BAND_CENTER_30M,    10125000).
%% -define(BAND_CENTER_40M,    7150000).
%% -define(BAND_CENTER_60M,    5357000).
%% -define(BAND_CENTER_80M,    3750000).
%% -define(BAND_CENTER_160M,   1900000).
    ...
    Response = case Action of
                   <<"fnctrl">> ->
                       ...
                       {<<"txspec">>, TxSpec} = lists:keyfind(<<"txspec">>, 1, Decoded),
                                                ...
                                                % lager:info("TxSpec=~p TxMode=~p Tx=~p RxSpec=~p RxMode=~p Rx=~p", [TxSpec,TxMode,Tx,RxSpec,RxMode,Rx]),
                   ...
                   <<"modemctrl">> ->
                       ...
                       TxGainResult = case lists:keyfind(<<"txgain">>, 1, Decoded) of
                                          {<<"txgain">>, TxGain} ->
                                              ...
                                              {atomic, ok} = modem_db:write_config("default_tx_gain", TxGainWord),
                       ...
                       AleTxGainResult = case lists:keyfind(<<"aletxgain">>, 1, Decoded) of
                                             {<<"aletxgain">>, AleTxGain} ->
                                                ...
                                                % ok = frontend_port:set_aletxgain(AleTxGainWord),
                                                 ...
                                                 {atomic, ok} = modem_db:write_config("ale_tx_gain", AleTxGainWord),
                       ...
                       RxGainResult = case lists:keyfind(<<"rxgain">>, 1, Decoded) of
                                          {<<"rxgain">>, RxGain} ->
                                              ...
                                              {atomic, ok} = modem_db:write_config("default_rx_gain", RxGainWord),
                   ...
                   <<"radioctrl_set">> ->
                       ...
                       build_response([{action,<<"radioctrl_set">>}, {response,<<"ok">>}, BandResult,FreqResult,ModeResult,PttResult]);
                   <<"channel_request">> ->
                       RecordResult = case lists:keyfind(<<"number">>, 1, Decoded) of
                                          {<<"number">>, ChanNum} ->
                                              lager:debug("Req channel ~p", [ChanNum]),                
                                              {ok, Record} = modem_db:read_channel(ChanNum),
                   ...
                   <<"contact_request">> ->
                       ...
                       ContactList = modem_db:read_all_contacts(),
                                                % SortedList = lists:reverse(lists:keysort(3,ContactList)),
                   ...
                   <<"config">> ->
                                                % client is setting modem configuration parameters
                       {<<"name">>, Name} = lists:keyfind(<<"name">>, 1, Decoded),
                       {<<"value">>, Value} = lists:keyfind(<<"value">>, 1, Decoded),
                       {atomic, ok} = modem_db:write_config(Name,Value),
...
websocket_info({timeout, _Ref, Msg}, Req, State) ->
    lager:info("client connection timeout..."),
                                                % ok = frontend_port:claddr({0,0,0,0}), %% no longer a valid IP to send to
