...
int kx3_poll_wait(radioif_t *rif)
{
	...
	if (ret == 0) {
		syslog(LOG_INFO, "timeout on kx3 serial wait\n");
		return -1;
	}
...
int kx3_set_band(radioif_t *rif, int band)
{
	...
	// syslog(LOG_INFO,"kx3_control: %s called (%d)\n", __PRETTY_FUNCTION__, band);

	// simple translation to KX3 band numbers (-1)
	sprintf(buf, "BN%02d;", band-1);	
	len = strlen(buf);
...
int kx3_set_freq(radioif_t *rif, int freq)
{
	...
	if (kx3_get_freq(rif, &rd_freq)) {
		syslog(LOG_INFO, "failed to validate frequency\n");
		return -1;
	}
	if (rd_freq == freq) {
		return 0;
...
int kx3_set_ptt(radioif_t *rif, int state)
{
	...
	for (retries = 0; retries < KX3_MAX_RETRIES; retries++) {
		if (kx3_get_ptt(rif, &rd_state)) {
			syslog(LOG_INFO,"failed to validate ptt\n");
			return -1;
		}
		if (rd_state == state) {
			return 0;
		...
		// syslog("retry...\n");
		usleep(500000);
		// set the PTT state again (paranoid perhaps)
		// ret = write(rif->handle, buf, len);
		// if (ret < len) {
		// 	syslog(LOG_INFO, "failure in write\n");
		// 	return -1;
...
int kx3_set_mode(radioif_t *rif, int state)
{
	...
	if (kx3_get_mode(rif, &rd_state)) {
		syslog(LOG_INFO,"failed to validate mode\n");
		return -1;
	}
	return 0;	
}
...
int kx3_control_init(radioif_t *rif)
{
	...
	if ((rif->handle = open_port(device,38400)) < 0) {
